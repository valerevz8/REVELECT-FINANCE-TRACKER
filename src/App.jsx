...
function FinanceApp({ session, theme, onThemeChange, subInfo }) {
  ...
  async function markUpcomingPaid(item) {
    if (item.source === 'recurring') {
      ...
      if (!dueErr) setRecurringItems(prev => prev.map(r => r.id === source.id ? { ...r, nextDueDate: nextDue } : r));
      else console.error('Failed to advance next_due_date after Mark as Paid:', dueErr);
    } else if (item.source === 'debt') {
      const did = item.id.startsWith('d-') ? item.id.slice(2) : item.debtId;
      await addTransaction({ type: 'debt_payment', date: todayISO(), category: null, debtId: did, accountId: null, amount: item.amount, note: t.debtPaymentDueLabel });
                  ...
                  {upcoming.slice(0, 4).map(r => {
                    ...
                    return (
                      <div key={r.id} className="fc-upcoming-item">
                        ...
                        {onMarkPaid && <button type="button" className="fc-mark-paid-btn" onClick={() => onMarkPaid(r)}>{t.markAsPaid}</button>}
                      </div>
                    );
                  })}
                </div>
