...
const T = {
  id: {
    ...
    categories: {
      ...
      subscription: 'Langganan', work: 'Kerja', other_expense: 'Lainnya',
  ...
  en: {
    ...
    categories: {
      ...
      subscription: 'Subscription', work: 'Work', other_expense: 'Other',
...
const EXPENSE_CATS = ['food', 'rent', 'transport', 'entertainment', 'bills', 'utilities', 'health', 'gym', 'shopping', 'vacation', 'education', 'family', 'pets', 'subscription', 'work', 'other_expense'];
...
// Richer, more saturated "premium colorful" palette — jewel tones instead of pale pastels,
// each still desaturated just enough to stay elegant rather than neon.
const CAT_COLORS = {
  food: '#D9A02E',         // rich mustard
  rent: '#C1603A',         // deep terracotta
  transport: '#6E5AC4',    // rich indigo-violet
  entertainment: '#D6487A',// rich rose/magenta
  bills: '#2E7FC1',        // rich blue
  utilities: '#1E9E93',    // deep teal-cyan
  health: '#3F9E52',       // rich green
  gym: '#0F9E86',          // deep emerald teal
  shopping: '#E0752A',     // rich orange
  vacation: '#E8955A',     // coral orange
  education: '#4C6FC1',    // indigo blue
  family: '#C1547A',       // berry rose
  pets: '#8154C1',         // rich purple
  subscription: '#3E8C7E', // deep sage teal
  work: '#A9822E',         // bronze khaki
  other_expense: '#7A7166',// warm taupe (deliberately muted — catch-all)
  salary: '#3F9E52', freelance: '#2E7FC1', trading: '#1E9E93', content: '#8154C1', other_income: '#7A7166',
  gold: '#B8863E', crypto: '#E0752A', stocks: '#2E7FC1', deposito: '#3E8C7E', reksadana: '#4C6FC1', other_investment: '#7A7166',
};
...
const CAT_ICONS = {
  ...
  family: Users, pets: PawPrint, subscription: Repeat, work: Briefcase, other_expense: MoreHorizontal,
...
export default function App() {
  ...
  if (session === undefined) {
    ...
    const fg = theme === 'light' ? '#6b4c26' : '#9c7a45';
...
function FinanceApp({ session, theme, onThemeChange, subInfo }) {
  ...
  const health = useMemo(() => {
    ...
    else if (score >= 60) { label = t.fairlyHealthy; color = '#9c7a45'; }
  ...
  if (!loaded) {
    return (
      <div style={{ background: theme === 'light' ? '#f7f3ea' : '#0d0b09', color: theme === 'light' ? '#6b4c26' : '#9c7a45', height: '100vh', display: 'flex', alignItems: 'center', justifyContent: 'center', fontFamily: 'Calibri, sans-serif' }}>
        {t.loadingApp}
      </div>
    );
  }
  ...
  return (
    <div className="fc-app" data-theme={theme}>
      ...
      <div className="fc-body fc-body-with-nav">
        ...
        {tab === 'charts' && (
          <>
            ...
            {analyticsSubTab === 'charts' ? <Charts t={t} totals={totals} lang={lang} transactions={transactions} /> : <MonthlyReport t={t} lang={lang} rows={monthlyRows} transactions={transactions} debts={debts} />}
          </>
        )}
        {tab === 'review' && (
      ...
      <AskReviButton t={t} lang={lang} snapshot={{
        month: monthLabel,
...
function AskReviButton({ t, lang, snapshot }) {
    ...
    <>
      ...
      {open && (
        <div className="fc-modal-backdrop" onClick={() => setOpen(false)}>
          <div className="fc-modal fc-modal-advisor" onClick={(e) => e.stopPropagation()}>
            <div className="fc-modal-header">
            ...
            <AdvisorChat t={t} lang={lang} snapshot={snapshot} />
          </div>
        </div>
      )}
    </>
...
function StatCard({ label, value, icon: Icon, accent }) {
  return (
    <div className="fc-card fc-stat">
      <div className="fc-stat-icon" style={{ color: accent || '#9c7a45' }}><Icon size={18} /></div>
...
function AnalyticsTrend({ t, lang, transactions }) {
  ...
  const buckets = useMemo(() => {
    ...
    if (period === 'week') {
      ...
      for (let i = 6; i >= 0; i--) {
        ...
        days.push({ key: d.toISOString().slice(0, 10), label: dtf({ weekday: 'short' }).format(d) });
      }
      return days;
    }
    if (period === 'month') {
  ...
  const data = useMemo(() => {
    return buckets.map(b => {
      ...
      return { label: b.label, expense, income, net: income - expense };
    });
    ...
    <div className="fc-card fc-analytics-card">
      ...
      <ResponsiveContainer width="100%" height={240}>
        <ComposedChart data={data} margin={{ left: 0, right: 8, top: 8, bottom: 0 }}>
          ...
          <XAxis dataKey="label" tick={{ fill: 'var(--fc-text-dim)', fontSize: 11 }} axisLine={{ stroke: 'var(--fc-border)' }} tickLine={false} />
          ...
          <Bar dataKey="expense" name={t.expensesLabel} fill="#b0584f" fillOpacity={0.82} radius={[4, 4, 0, 0]} barSize={period === 'week' ? 18 : 12} />
          <Bar dataKey="income" name={t.incomeLabel} fill="#7c9a72" fillOpacity={0.82} radius={[4, 4, 0, 0]} barSize={period === 'week' ? 18 : 12} />
          <Line type="monotone" dataKey="expense" name={`${t.expensesLabel} · ${t.trendLabel}`} stroke="var(--fc-gold-bright, #9c7a45)" strokeWidth={2} dot={false} activeDot={{ r: 3 }} isAnimationActive={false} legendType="none" />
          <Line type="monotone" dataKey="income" name={`${t.incomeLabel} · ${t.trendLabel}`} stroke="var(--fc-gold, #9c7a45)" strokeWidth={2} strokeDasharray="4 3" dot={false} activeDot={{ r: 3 }} isAnimationActive={false} legendType="none" />
        </ComposedChart>
      </ResponsiveContainer>
      <div className="fc-analytics-legend">
        <div className="fc-analytics-legend-item"><i style={{ background: '#b0584f' }} /><span>{t.expensesLabel}</span></div>
        ...
        <div className="fc-analytics-legend-item"><i className="fc-analytics-legend-line" style={{ background: 'var(--fc-gold)', opacity: 0.6 }} /><span>{t.incomeLabel} {t.trendLabel}</span></div>
      </div>
      <div className="fc-analytics-deltas">
        <div><span className="fc-delta-inline" style={{ color: expenseDelta >= 0 ? '#b0584f' : '#7c9a72' }}>{expenseDelta >= 0 ? '+' : ''}{expenseDelta.toFixed(0)}%</span> {t.expensesLabel} {t.vsPreviousPeriod}</div>
        <div><span className="fc-delta-inline" style={{ color: incomeDelta >= 0 ? '#7c9a72' : '#b0584f' }}>{incomeDelta >= 0 ? '+' : ''}{incomeDelta.toFixed(0)}%</span> {t.incomeLabel} {t.vsPreviousPeriod}</div>
      </div>
    </div>
  );
}

function Charts({ t, totals, lang, transactions }) {
  ...
  return (
    <div className="fc-charts-grid">
      <AnalyticsTrend t={t} lang={lang} transactions={transactions} />
      <div className="fc-card">
          ...
          <div className="fc-pie-row">
            <div className="fc-donut-wrap">
              <ResponsiveContainer width="100%" height={220}>
                <PieChart>
                  <Pie data={expenseData} dataKey="value" nameKey="name" cx="50%" cy="50%" innerRadius={58} outerRadius={84}
...
function Review({ t, lang, transactions, monthTx, totals, health, selectedMonth, debts, budgets, investPlan, investBudgets, totalDebtRemaining, monthLabel, accounts, onAddAccount, onEditAccount, onDeleteAccount, onEditInvestPlan }) {
    ...
    <div className="fc-review">
            ...
            <div className="fc-tips">
              ...
              {tips.map((tip, i) => (
                <div key={i} className="fc-tip-item">
                  {tips.length === 1 ? <CheckCircle2 size={15} color="#7c9a72" /> : <AlertCircle size={15} color="#9c7a45" />}
                  <span>{tip}</span>
                </div>
              ))}
            </div>
      ...
      <div className="fc-grid-2">
        ...
        <div className="fc-stat-grid">
          ...
          <StatCard label={t.investable} value={fmtIDR(totals.investable)} icon={Wallet} accent="#9c7a45" />
...
function MetricBar({ label, value, invert }) {
  ...
  const color = good ? '#7c9a72' : (invert ? (value < 0.5 ? '#9c7a45' : '#b0584f') : (value > 0.05 ? '#9c7a45' : '#b0584f'));
  return (
        ...
        {type === 'debt_payment' ? (
          ...
          <label className="fc-field"><span>{t.category}</span>
            <select value={category} onChange={(e) => setCategory(e.target.value)}>
              ...
              {type !== 'income' && <option value="__custom__">{t.addCustomCategory}</option>}
            </select>
          </label>
        )}
...
.fc-app[data-theme="dark"], .fc-authwrap[data-theme="dark"], .fc-wizard[data-theme="dark"] {
  --fc-bg: #0d0b09; --fc-surface: #17130f; --fc-surface2: #1f1a15; --fc-border: #3a3226;
  --fc-text: #f0ece4; --fc-text-dim: #9b9186; --fc-gold-bright: #c2a066; --fc-border-strong: #4a4032;
}
.fc-app[data-theme="light"], .fc-authwrap[data-theme="light"], .fc-wizard[data-theme="light"] {
  ...
  --fc-text: #2b2418; --fc-text-dim: #78705f; --fc-gold-bright: #6b4c26; --fc-border-strong: #cbbd9c;
}
...
.fc-avatar { border-radius: 50%; background: var(--fc-gold-bright, #9c7a45); color: var(--fc-bg); display: flex; align-items: center; justify-content: center; font-weight: 700; font-family: 'Calibri','Segoe UI',sans-serif; flex-shrink: 0; }
...
.fc-lang-active { color: #9c7a45; font-weight: 700; }
.fc-lang-sep { opacity: 0.4; }

/* Bottom navigation dock — mobile fixed, desktop compact floating dock */
.fc-bottomnav { position: fixed; left: 0; right: 0; bottom: 0; z-index: 45; display: flex; align-items: center; justify-content: space-around; gap: 4px; background: var(--fc-surface); border-top: 1px solid var(--fc-border); padding: 8px max(12px, env(safe-area-inset-left)) max(8px, env(safe-area-inset-bottom)); }
...
.fc-bottomnav-active { color: var(--fc-gold-bright, #9c7a45); background: rgba(201,169,119,0.1); }
.fc-bottomnav-action { background: var(--fc-gold-bright, #9c7a45); border: none; color: var(--fc-bg); width: 50px; height: 50px; border-radius: 50%; display: flex; align-items: center; justify-content: center; cursor: pointer; flex-shrink: 0; margin: 0 4px; box-shadow: 0 4px 14px rgba(201,169,119,0.35); transition: transform 0.2s ease; }
...
.fc-monthnav button { background: var(--fc-surface); border:1px solid var(--fc-border); color:#9c7a45; border-radius:6px; padding:5px 8px; cursor:pointer; display:flex; }
...
/* Home V3 */
.fc-safe-hero { padding: 18px 16px 20px; border: 1px solid var(--fc-border); background: var(--fc-surface); border-radius: 14px; margin-bottom: 18px; }
...
.fc-revi-mark { display: inline-flex; align-items: center; justify-content: center; width: 22px; height: 22px; border-radius: 50%; background: var(--fc-gold-bright, #9c7a45); color: var(--fc-bg); font-weight: 800; font-size: 12px; font-family: 'Calibri','Segoe UI',sans-serif; flex-shrink: 0; }
...
.fc-advisor-msg-user { align-self: flex-end; background: var(--fc-gold, #9c7a45); color: #14100c; border-bottom-right-radius: 4px; }
...
.fc-analytics-card { grid-column: 1 / -1; }
...
.fc-btn-primary { display:flex; align-items:center; gap:6px; background: #9c7a45; color: #14100c; border:none; padding: 9px 14px; border-radius: 8px; font-size: 13px; font-weight:700; cursor:pointer; }
...
.fc-debt-bar { height:100%; background: linear-gradient(90deg, #9c7a45, #4f8a8b); }
.fc-debt-meta { display:flex; justify-content:space-between; font-size: 12px; color: var(--fc-text-dim); flex-wrap:wrap; gap:6px; }
.fc-greeting { font-size: 18px; font-weight: 600; color: var(--fc-gold-bright, #9c7a45); margin-bottom: 12px; font-family: 'Calibri','Segoe UI',sans-serif; }
...
.fc-section { margin-bottom: 18px; background: var(--fc-surface); border: 1px solid var(--fc-border); border-radius: 14px; padding: 16px; }
...
.fc-section-upcoming { background: var(--fc-surface2); border-color: var(--fc-border-strong, var(--fc-border)); }
.fc-section-light { background: transparent; border-style: dashed; }
...
.fc-spend-bar-fill { height: 100%; background: var(--fc-gold-bright, #9c7a45); border-radius: 3px; }
...
.fc-advisor-mic-active { color: #b0584f; border-color: #b0584f; background: rgba(176,88,79,0.08); animation: fcPulseMic 1.4s ease-in-out infinite; }
...
.fc-type-btn-active { border-color: #9c7a45; color: #9c7a45; background: rgba(201,169,119,0.08); }
...
.fc-split-final-row { display: flex; justify-content: space-between; align-items: center; margin-top: 8px; padding-top: 10px; border-top: 1px solid var(--fc-border-strong, var(--fc-border)); font-size: 15px; font-weight: 700; color: var(--fc-gold-bright, #9c7a45); }
...
.fc-field input:focus, .fc-field select:focus { outline: 1.5px solid #9c7a45; }
...
.fc-password-eye:hover { color: #9c7a45; }
.fc-link-btn { background: none; border: none; color: #9c7a45; font-size: 12px; text-align: left; cursor: pointer; padding: 0; margin-top: -4px; text-decoration: underline; text-underline-offset: 2px; }
