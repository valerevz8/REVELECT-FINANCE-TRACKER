...
function FinanceApp({ session, theme, onThemeChange, subInfo }) {
  ...
  async function saveDisplayName(name) {
    ...
    await supabase.from('finance_settings').upsert({ user_id: userId, display_name: name }, { onConflict: 'user_id' });
  }
  async function skipNamePrompt() {
    setNamePromptSkipped(true);
    setShowNamePrompt(false);
    await supabase.from('finance_settings').upsert({ user_id: userId, name_prompt_skipped: true }, { onConflict: 'user_id' });
  }
