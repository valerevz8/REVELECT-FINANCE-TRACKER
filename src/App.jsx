...
function FinanceApp({ session, theme, onThemeChange }) {
  ...
  return (
    <div className="fc-app" data-theme={theme}>
      ...
      {showSplitBillModal && (
        <SplitBillModal t={t} onClose={() => setShowSplitBillModal(false)}
...
function QuickAddModal({ t, onClose, onParsed }) {
  ...
  function handleFile(file) {
    ...
    reader.onload = () => {
      ...
      const mt = meta.match(/data:(.*);base64/)?.[1] || file.type;
...
function SplitBillModal({ t, onClose, onParsed }) {
  ...
  async function handleSplit(e) {
    ...
    try {
      ...
      onParsed({
        ...
        amount: Number(data.amount) || '',
        date: todayISO(),
        note: data.note || (Array.isArray(data.items) ? data.items.join(', ') : ''),
        debtId: null,
      });
    } catch (err) {
      setError(t.parseFailed);
