...
function Dashboard({ t, totals, displayName, budgets, accounts, recurringItems, debts, monthlyRows, onViewAnalytics, onViewTransactions, transactions, monthTx, selectedMonth, onEditBudget }) {
  ...
  const upcoming = (recurringItems || [])
    .filter(r => r.active && r.type !== 'income' && r.nextDueDate >= today && r.nextDueDate <= in14ISO)
  ...
  upcoming.forEach(r => { committedByCat[r.category] = (committedByCat[r.category] || 0) + Number(r.amount || 0); });
                  ...
                  {upcoming.slice(0, 4).map(r => {
                    ...
                    return (
                      <div key={r.id} className="fc-upcoming-item">
                        ...
                        <div className="fc-upcoming-due">{relativeDay(r.nextDueDate)}</div>
                      </div>
                    );
                  })}
                </div>
