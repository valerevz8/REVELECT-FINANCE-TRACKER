...
const T = {
  id: {
    ...
    wizNameTitle: 'Siapa nama kamu?', wizNameDesc: 'Biar Revinance bisa manggil kamu, bukan cuma nunjukkin angka.', wizNamePlaceholder: 'Nama kamu', wizNameCta: 'Lanjut',
    wizSpaceTitle: 'Hai {name}, kasih nama Space kamu.', wizSpaceDesc: 'Space itu tempat kamu misahin satu sisi hidup finansial — pribadi, kerja sampingan, atau bisnis.',
    wizSpaceLabel: 'Space', wizSpaceDefault: "Space {name}", wizLanguageLabel: 'Bahasa', wizCurrencyLabel: 'Mata Uang', wizBack: 'Kembali', wizFinish: 'Masuk Aplikasi',
  ...
  en: {
    ...
    wizNameTitle: "What's your name?", wizNameDesc: "So Revinance can talk to you, not just show you numbers.", wizNamePlaceholder: 'Your name', wizNameCta: 'Continue',
    wizSpaceTitle: 'Hi {name}, name your Space.', wizSpaceDesc: 'A Space is where you separate one money life — personal, side jobs, or a business.',
    wizSpaceLabel: 'Space', wizSpaceDefault: "{name}'s Space", wizLanguageLabel: 'Language', wizCurrencyLabel: 'Currency', wizBack: 'Back', wizFinish: 'Enter App',
...
function FinanceApp({ session, theme, onThemeChange, subInfo }) {
  ...
  async function finishOnboardingWizard({ name, space, newLang, newCurrency }) {
    ...
    if (error) console.error('finishOnboardingWizard failed:', error);
  }
  ...
  return (
    <div className="fc-app" data-theme={theme}>
      ...
      <div className="fc-body">
        {tab === 'dashboard' && (
          <Dashboard t={t} totals={totals} health={health} totalDebtRemaining={totalDebtRemaining} lang={lang} displayName={displayName}
            budgets={budgets} onEditBudget={() => setShowBudgetModal(true)}
            investPlan={investPlan} onEditInvestPlan={() => setShowInvestPlanModal(true)}
      ...
      <div className="fc-fab-wrap">
        {showFabMenu && (
          <div className="fc-fab-menu">
            ...
            <button className="fc-fab-mini fc-fab-mini-primary" style={{ animationDelay: '80ms' }} onClick={() => { setShowFabMenu(false); setEditingTx(null); setShowTxModal(true); }}>
        ...
        <button className={`fc-fab-main ${showFabMenu ? 'fc-fab-main-open' : ''}`} onClick={() => setShowFabMenu(s => !s)} aria-label={t.addTransaction}>
          <Plus size={24} className="fc-fab-icon" />
        </button>
      </div>
    </div>
  );
...
function Dashboard({ t, totals, health, totalDebtRemaining, lang, displayName, budgets, onEditBudget, investPlan, onEditInvestPlan, accounts, onAddAccount, onEditAccount, onDeleteAccount }) {
        ...
        {pieData.length === 0 ? <div className="fc-empty">{t.noData}</div> : (
          <div className="fc-pie-row">
            <ResponsiveContainer width="100%" height={230}>
              <PieChart>
                <Pie data={pieData} dataKey="value" nameKey="name" cx="50%" cy="50%" innerRadius={48} outerRadius={76} paddingAngle={2}
                  label={makePieIconLabel(pieData)} labelLine={false}>
...
function Review({ t, lang, transactions, monthTx, totals, health, selectedMonth, debts, budgets, investPlan, totalDebtRemaining, monthLabel }) {
  const now = new Date();
...
function OnboardingWizard({ t, theme, lang, onFinish }) {
  ...
  function goStep2(e) {
    ...
    if (!trimmed) return;
    if (!spaceTouched) setSpace(t.wizSpaceDefault.replace('{name}', trimmed));
    setStep(2);
  }

  function finish(e) {
    e.preventDefault();
    const trimmedSpace = space.trim() || t.wizSpaceDefault.replace('{name}', name.trim());
    onFinish({ name: name.trim(), space: trimmedSpace, newLang: wizLang, newCurrency: currency });
  }
    ...
    <div className="fc-wizard" data-theme={theme}>
      ...
      {step === 2 && (
        <form className="fc-wizard-screen" onSubmit={finish}>
          <div className="fc-wizard-body">
            <h1 className="fc-wizard-title">{t.wizSpaceTitle.replace('{name}', name.trim())}</h1>
            <p className="fc-wizard-desc">{t.wizSpaceDesc}</p>

            <label className="fc-wizard-field-label">{t.wizSpaceLabel}</label>
            <input
              type="text" className="fc-wizard-input" value={space}
              onChange={(e) => { setSpace(e.target.value); setSpaceTouched(true); }}
              placeholder={t.wizSpaceDefault.replace('{name}', name.trim())}
            />
          ...
          <div className="fc-wizard-actions">
            <button type="button" className="fc-btn-ghost fc-wizard-back" onClick={() => setStep(1)}>{t.wizBack}</button>
            <button type="submit" className="fc-btn-primary fc-wizard-cta">{t.wizFinish}</button>
          </div>
        </form>
      )}
    </div>
...
.fc-fab-main-open:hover { transform: rotate(135deg) scale(1.14); }
.fc-fab-main-open { transform: rotate(135deg) scale(1.06); box-shadow: 0 8px 22px rgba(0,0,0,0.5); }
...
.fc-wizard-title { font-family: 'Cinzel','Calibri',serif; font-size: 30px; font-weight: 600; line-height: 1.25; color: var(--fc-text); margin: 0 0 6px; letter-spacing: 0.3px; }
