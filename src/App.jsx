...
const T = {
  id: {
    ...
    settingsProfile: 'PROFIL', profileName: 'Nama', spaceLabel: 'Space',
    settingsMoney: 'KEUANGAN', settingsGoals: 'Target', settingsBudget: 'Kategori & Budget', settingsRecurring: 'Transaksi Berulang', settingsReminders: 'Pengingat',
    ...
    settingsData: 'DATA', settingsImportExport: 'Import / Export',
    ...
    todayDetailTitle: 'Detail Hari Ini', noSpendingToday: 'Ga ada pengeluaran hari ini.',
    ...
    remindMe: 'Ingetin Gw', debtPaymentDueLabel: 'Bayar Debt',
  ...
  en: {
    ...
    calendarTitle: 'Expense Calendar',
    ...
    settingsProfile: 'PROFILE', profileName: 'Name', spaceLabel: 'Space',
    settingsMoney: 'MONEY', settingsGoals: 'Goals', settingsBudget: 'Category & Budget', settingsRecurring: 'Recurring Transactions', settingsReminders: 'Reminders',
    ...
    todayDetailTitle: 'Today Detail', noSpendingToday: 'No spending today.',
...
function FinanceApp({ session, theme, onThemeChange, subInfo }) {
  ...
  return (
    <div className="fc-app" data-theme={theme}>
      ...
      <div className="fc-body fc-body-with-nav">
        {tab === 'dashboard' && (
          <Dashboard t={t} lang={lang} totals={totals} displayName={displayName} budgets={budgets} accounts={accounts}
            ...
            transactions={transactions} monthTx={monthTx} selectedMonth={selectedMonth} onEditBudget={() => setShowBudgetModal(true)} />
        )}
        {tab === 'transactions' && (
          <>
        ...
        {tab === 'review' && (
          <Review t={t} lang={lang} transactions={transactions} monthTx={monthTx} totals={totals} health={health} selectedMonth={selectedMonth}
            ...
            onEditInvestPlan={() => setShowInvestPlanModal(true)} />
        )}
        {tab === 'report' && (
      ...
      {showDebtModal && (
        <DebtModal t={t} lang={lang} editingDebt={editingDebt}
...
function BottomNav({ t, tab, setTab }) {
  const items = [
    ...
    { key: 'charts', icon: PieIcon }, { key: 'review', icon: ClipboardList },
...
function Dashboard({ t, lang, totals, displayName, budgets, accounts, recurringItems, debts, monthlyRows, onViewAnalytics, onViewTransactions, onMarkPaid, onOpenDebtPay, transactions, monthTx, selectedMonth, onEditBudget }) {
  const now = new Date();
  ...
  const todayByCat = {};
  todayExpenseTx.forEach(tx => { todayByCat[tx.category] = (todayByCat[tx.category] || 0) + Number(tx.amount); });
  ...
  const weekTx = (transactions || []).filter(tx => (tx.type === 'expense' || tx.type === 'debt_payment') && inRange(tx.date, weekAgo, now));
  ...
  if (todayTopCats.length > 0) {
    ...
    if (topVal / todayExpenseTotal >= 0.5) {
      let sentence = t.insightCatConcentration.replace('{cat}', t.categories[topCat] || topCat);
      if (Number(budgets[topCat]) > 0) {
        const pct = (homeByCatExpense[topCat] || 0) / Number(budgets[topCat]);
        sentence += pct > 1 ? t.insightBudgetOver.replace('{cat}', t.categories[topCat] || topCat)
          : pct >= 0.8 ? t.insightBudgetNear.replace('{cat}', t.categories[topCat] || topCat)
          : t.insightBudgetSafe.replace('{cat}', t.categories[topCat] || topCat);
      }
      ...
      {isEmpty ? (
        ...
        <>
          ...
          <div className="fc-safe-hero">
            ...
            <div className="fc-safe-amount" style={{ color: overCommitted ? '#b0584f' : 'var(--fc-text)' }}>{fmtIDR(safeToSpend)}</div>
            <div className="fc-safe-bar-track">
              <div className="fc-safe-bar-seg" style={{ width: `${committedPct}%`, background: '#b0584f' }} />
              <div className="fc-safe-bar-seg" style={{ width: `${reservedPct}%`, background: '#c98f4f' }} />
              <div className="fc-safe-bar-seg" style={{ width: `${safePct}%`, background: 'var(--fc-gold-bright)' }} />
            </div>
            <div className="fc-safe-legend">
              <span><i style={{ background: 'var(--fc-text)' }} />{t.balanceLabel}: {fmtIDR(netWorth)}</span>
              <span><i style={{ background: '#b0584f' }} />{t.committedLabel}: {fmtIDR(committedUpcoming)}</span>
              <span><i style={{ background: '#c98f4f' }} />{t.reservedLabel}: {fmtIDR(reservedBudget)}</span>
            </div>
          </div>
                  ...
                  return (
                    <div key={c} className="fc-budget-row">
                      ...
                      <div className="fc-safe-bar-track" style={{ height: 5 }}>
                        <div className="fc-safe-bar-seg" style={{ width: `${pct}%`, background: over ? '#b0584f' : color }} />
                      </div>
          ...
          <div className="fc-grid-2">
                ...
                <div className="fc-mini-cal-detail">
                  {selectedDayTx.length === 0 ? t.calendarNoTx : (
                    <span>
                      ...
                      {selectedDayExpense > 0 && <b style={{ color: '#b0584f' }}>-{fmtIDR(selectedDayExpense)}</b>}
                    </span>
                  )}
                </div>
              )}
            ...
            <div className="fc-section">
              <div className="fc-section-header">
                <div className="fc-section-title">{t.todayDetailTitle}</div>
                {onViewTransactions && <button className="fc-link-btn" onClick={onViewTransactions}>{t.viewAllLink}</button>}
              </div>
              {todayTxCount === 0 ? (
                <div className="fc-section-empty">{t.noSpendingToday}</div>
              ) : (
                <>
                  <div className="fc-today-rows">
                    {todayIncomeTotal > 0 && <div className="fc-today-row"><span>{t.incomeLabel}</span><b style={{ color: '#7c9a72' }}>+{fmtIDR(todayIncomeTotal)}</b></div>}
                    <div className="fc-today-row"><span>{t.expensesLabel}</span><b style={{ color: '#b0584f' }}>-{fmtIDR(todayExpenseTotal)}</b></div>
                    <div className="fc-today-row fc-today-net"><span>{t.netLabel}</span><b style={{ color: todayNet >= 0 ? '#7c9a72' : '#b0584f' }}>{todayNet >= 0 ? '+' : ''}{fmtIDR(todayNet)}</b></div>
                  </div>
                  {todayTopCats.length > 0 && (
                    <div className="fc-today-cats">
                      {todayTopCats.map(([cat, val]) => (
...
function Review({ t, lang, transactions, monthTx, totals, health, selectedMonth, debts, budgets, investPlan, totalDebtRemaining, monthLabel, accounts, onAddAccount, onEditAccount, onDeleteAccount, onEditInvestPlan }) {
  ...
  return (
    <div className="fc-modal-backdrop" onClick={onClose}>
      <div className="fc-account-panel" onClick={(e) => e.stopPropagation()}>
        ...
        <div className="fc-account-name">{spaceName || (displayName ? `${displayName}'s Space` : t.account)}</div>
...
function SettingsRow({ label, onClick, comingSoon, t }) {
  return (
    <button type="button" className="fc-settings-row" onClick={comingSoon ? undefined : onClick} disabled={comingSoon}>
      <span>{label}</span>
      {comingSoon ? <span className="fc-settings-soon">{t.comingSoon}</span> : <ChevronRight size={15} />}
    </button>
  );
}

function SettingsPanel({ t, lang, setLang, theme, onThemeChange, displayName, spaceName, onSaveProfile, onClose, onOpenBudget, onOpenRecurring }) {
    ...
    <div className="fc-modal-backdrop" onClick={onClose}>
      <div className="fc-settings-panel" onClick={(e) => e.stopPropagation()}>
        ...
        <div className="fc-settings-scroll">
          ...
          <div className="fc-settings-block">
            <SettingsRow t={t} label={t.settingsGoals} comingSoon />
          ...
          <div className="fc-settings-block">
            ...
            <SettingsRow t={t} label={t.settingsNotifications} comingSoon />
            <div className="fc-settings-row" style={{ cursor: 'default' }}>
          ...
          <div className="fc-settings-block">
            <SettingsRow t={t} label={t.settingsImportExport} comingSoon />
          </div>

          <div className="fc-settings-group-title">{t.settingsSupport}</div>
          <div className="fc-settings-block">
            <SettingsRow t={t} label={t.settingsReview} comingSoon />
            <SettingsRow t={t} label={t.settingsFeatureRequest} comingSoon />
            <SettingsRow t={t} label={t.settingsPrivacy} comingSoon />
            <SettingsRow t={t} label={t.settingsTerms} comingSoon />
          </div>
        </div>
      </div>
    </div>
...
function DebtModal({ t, lang, editingDebt, onClose, onSave }) {
  ...
  function handleSubmit(e) {
    ...
    const payload = {
      ...
      dueDate: dueDate || null, reminders: dueDate ? reminders : [],
    };
  ...
  return (
    <div className="fc-modal-backdrop" onClick={onClose}>
      <form className="fc-modal" onClick={(e) => e.stopPropagation()} onSubmit={handleSubmit}>
        ...
        {dueDate && <ReminderPicker t={t} lang={lang} value={reminders} onChange={setReminders} />}
...
.fc-avatar { border-radius: 50%; background: var(--fc-gold-bright, #c9a977); color: var(--fc-bg); display: flex; align-items: center; justify-content: center; font-weight: 700; font-family: 'Calibri','Segoe UI',sans-serif; flex-shrink: 0; }
...
.fc-bottomnav { position: fixed; left: 50%; transform: translateX(-50%); bottom: max(14px, env(safe-area-inset-bottom)); z-index: 45; display: flex; gap: 4px; background: var(--fc-surface); border: 1px solid var(--fc-border); border-radius: 22px; padding: 6px; box-shadow: 0 8px 24px rgba(0,0,0,0.18); }
.fc-bottomnav-btn { background: none; border: none; color: var(--fc-text-dim); width: 44px; height: 44px; border-radius: 16px; display: flex; align-items: center; justify-content: center; cursor: pointer; }
.fc-bottomnav-btn:hover { color: var(--fc-text); }
.fc-bottomnav-active { color: var(--fc-bg); background: var(--fc-gold-bright, #c9a977); }
...
.fc-account-row-danger { color: #b0584f; }
...
.fc-settings-row { display: flex; align-items: center; justify-content: space-between; background: none; border: none; border-bottom: 1px solid var(--fc-border); color: var(--fc-text); padding: 13px 0; font-size: 13.5px; cursor: pointer; text-align: left; }
...
/* Home V3 */
.fc-safe-hero { padding: 18px 0 20px; border-bottom: 1px solid var(--fc-border); margin-bottom: 20px; }
...
.fc-period-card { background: var(--fc-surface); border: 1px solid var(--fc-border); border-radius: 12px; padding: 12px 13px; }
...
.fc-budget-over { color: #b0584f; font-weight: 600; }
...
.fc-mini-cal-today span { color: var(--fc-gold); font-weight: 700; }
.fc-mini-cal-selected { background: var(--fc-surface2); }
...
.fc-mini-cal-dot-expense { background: #b0584f; }
...
.fc-fab-wrap { position: fixed; right: 20px; bottom: max(78px, calc(env(safe-area-inset-bottom) + 68px)); z-index: 40; display: flex; flex-direction: column; align-items: flex-end; gap: 10px; }
...
.fc-upcoming-overdue { color: #b0584f; font-weight: 600; }
...
.fc-mark-paid-error { font-size: 10px; color: #b0584f; white-space: nowrap; }
...
.fc-reminder-chip-active { border-color: var(--fc-gold); color: var(--fc-gold); background: rgba(201,169,119,0.1); }
.fc-reminder-badge { display: inline-block; margin-left: 6px; font-size: 9.5px; font-weight: 700; color: var(--fc-gold); background: rgba(201,169,119,0.12); border-radius: 4px; padding: 1px 5px; vertical-align: middle; }
