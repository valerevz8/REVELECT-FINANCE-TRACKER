...
import { Plus, Trash2, TrendingUp, TrendingDown, Wallet, CreditCard, PiggyBank, PieChart as PieIcon, Table as TableIcon, Globe, X, ChevronLeft, ChevronRight, ClipboardList, LayoutDashboard, AlertCircle, CheckCircle2, LogOut, Eye, EyeOff, Sun, Moon, Pencil, Sparkles, Share2, FileText, Download } from 'lucide-react';
...
function FinanceApp({ session, theme, onThemeChange }) {
  ...
  async function updateTransaction(updated) {
    ...
    const { data, error } = await supabase.from('finance_transactions').update({
      date: updated.date, type: updated.type, category: updated.category, debt_id: updated.debtId, amount: updated.amount, note: updated.note,
  ...
  return (
    <div className="fc-app" data-theme={theme}>
      ...
      {showTxModal && (
        <TxModal t={t} lang={lang} debts={debts} presetDebtId={debtPayTargetId} editingTx={editingTx}
...
function Dashboard({ t, totals, health, totalDebtRemaining, lang, budgets, onEditBudget, investPlan, onEditInvestPlan }) {
...
function Transactions({ t, lang, monthTx, debts, onAdd, onQuickAdd, onSplitBill, onDelete, onEdit }) {
        ...
        {expenseData.length === 0 ? <div className="fc-empty">{t.noData}</div> : (
          <div className="fc-pie-row">
            <ResponsiveContainer width="100%" height={220}>
              <PieChart>
                <Pie data={expenseData} dataKey="value" nameKey="name" cx="50%" cy="50%" innerRadius={50} outerRadius={85} paddingAngle={2}>
...
/* ---------------------------------------------------------
   Modals
--------------------------------------------------------- */
function TxModal({ t, lang, debts, presetDebtId, editingTx, onClose, onSave }) {
  ...
  function handleSubmit(e) {
    ...
    const payload = { type, date, amount: Number(amount), note, category: type === 'debt_payment' ? null : category, debtId: type === 'debt_payment' ? debtId : null };
...
.fc-split-final-row span:last-child { font-family: ui-monospace, monospace; font-size: 19px; }
