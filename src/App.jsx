...
const T = {
  id: {
    ...
    dueToday: 'Hari ini', dueTomorrow: 'Besok', dueInDays: '{n} hari lagi',
  ...
  en: {
    ...
    dueToday: 'Today', dueTomorrow: 'Tomorrow', dueInDays: 'in {n} days',
...
function buildUpcomingList(recurringItems, debts, todayStr, windowDays) {
  ...
  (recurringItems || []).forEach(r => {
    if (!r.active || r.type === 'income' || r.nextDueDate < todayStr || r.nextDueDate > windowEndISO) return;
  ...
  (debts || []).forEach(d => {
    if (!d.dueDate || d.dueDate < todayStr || d.dueDate > windowEndISO) return;
  ...
  async function addTransaction(tx) {
    ...
    if (tx.type === 'debt_payment' && tx.debtId) {
      ...
      if (debt) {
        ...
        if (!uerr) setDebts(prev => prev.map(d => d.id === tx.debtId ? { ...d, remaining: newRemaining, dueDate: updatePayload.due_date === null ? null : d.dueDate } : d));
      }
    }
...
function Dashboard({ t, totals, displayName, budgets, accounts, recurringItems, debts, monthlyRows, onViewAnalytics, onViewTransactions, transactions, monthTx, selectedMonth, onEditBudget }) {
                  ...
                  {upcoming.slice(0, 4).map(r => {
                    ...
                    return (
                      <div key={r.id} className="fc-upcoming-item">
                        <div className="fc-upcoming-label">{label}{activeReminder && <span className="fc-reminder-badge">{activeReminder}</span>}</div>
                        <div className="fc-upcoming-amount">{fmtIDR(r.amount)}</div>
                        <div className="fc-upcoming-due">{relativeDay(r.dueDate)}</div>
                      </div>
                    );
                  })}
                </div>
              ...
              <div key={d.id} className="fc-debt-item">
                <div className="fc-debt-top">
                  <div className="fc-debt-name">
                    {d.name}
                    {activeReminder && <span className="fc-reminder-badge">{activeReminder}</span>}
                  </div>
...
function ReminderPicker({ t, value, onChange }) {
  function toggle(opt) {
  ...
  return (
    <div className="fc-modal-backdrop" onClick={onClose}>
      <form className="fc-modal" onClick={(e) => e.stopPropagation()} onSubmit={handleSubmit}>
        ...
        {dueDate && <ReminderPicker t={t} value={reminders} onChange={setReminders} />}
  ...
  return (
    <div className="fc-modal-backdrop" onClick={onClose}>
      <form className="fc-modal" onClick={(e) => e.stopPropagation()} onSubmit={handleSubmit}>
        ...
        <label className="fc-field"><span>{t.note}</span><input type="text" value={note} onChange={(e) => setNote(e.target.value)} /></label>
        {type !== 'income' && <ReminderPicker t={t} value={reminders} onChange={setReminders} />}
        {isEdit && (
...
.fc-upcoming-due { font-size: 11px; color: var(--fc-text-dim); flex-shrink: 0; width: 70px; text-align: right; }
