...
function Dashboard({ t, totals, displayName, budgets, accounts, recurringItems, debts, monthlyRows, onViewAnalytics, transactions, monthTx, selectedMonth, onEditBudget }) {
  ...
  const todayTx = (transactions || []).filter(tx => tx.type === 'expense' && tx.date === today);
  const todayTotal = todayTx.reduce((s, tx) => s + Number(tx.amount), 0);
  ...
  if (priorMonths.length > 0 && totals.expense > 0) {
    ...
    const daysInMonthN = endOfMonth.getDate();
  ...
  // Compact expense calendar for the selected month
  ...
  const dayTotals = {};
  monthExpenseTx.forEach(tx => { dayTotals[tx.date] = (dayTotals[tx.date] || 0) + Number(tx.amount); });
  const maxDayTotal = Math.max(0, ...Object.values(dayTotals));
  const cells = [];
  for (let i = 0; i < firstWeekday; i++) cells.push(null);
  for (let d = 1; d <= daysInMonth; d++) cells.push(d);
  const selectedDayTx = selectedDate ? monthExpenseTx.filter(tx => tx.date === selectedDate) : [];
  const selectedDayTotal = selectedDayTx.reduce((s, tx) => s + Number(tx.amount), 0);
        ...
        <>
          ...
          <div className="fc-period-grid">
            <div className="fc-period-card">
              ...
              <div className="fc-period-amount">{fmtIDR(todayTotal)}</div>
              <div className="fc-period-count">{t.txCount.replace('{n}', todayTx.length)}</div>
            </div>
            <div className="fc-period-card">
          ...
          <div className="fc-section">
            ...
            {activeBudgets.length === 0 ? (
              <div className="fc-section-empty">{t.noBudgetSet}</div>
            ) : (
          ...
          <div className="fc-section">
            ...
            <div className="fc-mini-calendar">
              ...
              {cells.map((d, i) => {
                ...
                const intensity = maxDayTotal > 0 ? clamp01(val / maxDayTotal) : 0;
                ...
                return (
                  <button key={i} type="button" className={`fc-mini-cal-cell ${isToday ? 'fc-mini-cal-today' : ''} ${isSelected ? 'fc-mini-cal-selected' : ''}`}
                    ...
                    <span>{d}</span>
                    {val > 0 && <i className="fc-mini-cal-dot" style={{ opacity: 0.35 + intensity * 0.65 }} />}
                  </button>
                );
              })}
            </div>
            {selectedDate && (
              <div className="fc-mini-cal-detail">
                {selectedDayTx.length === 0 ? t.calendarNoTx : (
                  <span>{selectedDate.slice(8, 10)}/{selectedDate.slice(5, 7)}: <b>{fmtIDR(selectedDayTotal)}</b> · {t.txCount.replace('{n}', selectedDayTx.length)}</span>
                )}
              </div>
            )}
          </div>
...
.fc-mini-cal-dot { width: 4px; height: 4px; border-radius: 50%; background: var(--fc-gold-bright); }
