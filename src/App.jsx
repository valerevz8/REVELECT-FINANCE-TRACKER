...
const T = {
  id: {
    ...
    markAsPaid: 'Tandai Lunas', markPaidError: 'Gagal, coba lagi.',
  ...
  en: {
    ...
    markAsPaid: 'Mark as Paid', markPaidError: 'Failed, try again.',
  ...
  async function markUpcomingPaid(item) {
    try {
      ...
      if (item.source === 'debt') {
        ...
        const result = await addTransaction({ type: 'debt_payment', date: todayISO(), category: null, debtId: did, accountId: null, amount: item.amount, note: t.debtPaymentDueLabel });
        if (result?.error) { console.error('markUpcomingPaid: debt addTransaction failed', result.error); return false; }
        // addTransaction already clears due_date when the payment date >= due_date; today always is.
        return true;
      }
      console.error('markUpcomingPaid: unknown item.source', item);
      return false;
    } catch (err) {
      ...
      <div className="fc-body">
        {tab === 'dashboard' && (
          <Dashboard t={t} lang={lang} totals={totals} displayName={displayName} budgets={budgets} accounts={accounts}
            ...
            onViewTransactions={() => setTab('transactions')} onMarkPaid={markUpcomingPaid}
...
function Dashboard({ t, lang, totals, displayName, budgets, accounts, recurringItems, debts, monthlyRows, onViewAnalytics, onViewTransactions, onMarkPaid, transactions, monthTx, selectedMonth, onEditBudget }) {
  ...
  const reservedBudget = activeBudgets.reduce((sum, c) => {
    const headroom = Math.max(0, Number(budgets[c]) - (totals.byCatExpense[c] || 0));
  ...
  const todayExpenseTx = (transactions || []).filter(tx => tx.type === 'expense' && tx.date === today);
  ...
  const weekTx = (transactions || []).filter(tx => tx.type === 'expense' && inRange(tx.date, weekAgo, now));
  const weekTotal = weekTx.reduce((s, tx) => s + Number(tx.amount), 0);
  const monthExpenseTx = (monthTx || []).filter(tx => tx.type === 'expense');
  ...
  const priorDaysExpense = (transactions || []).filter(tx => tx.type === 'expense' && tx.date !== today && new Date(tx.date) >= last30Start && new Date(tx.date) < now);
  ...
  if (todayTopCats.length > 0) {
    ...
    if (topVal / todayExpenseTotal >= 0.5) {
      ...
      if (Number(budgets[topCat]) > 0) {
        const pct = (totals.byCatExpense[topCat] || 0) / Number(budgets[topCat]);
  ...
  const insight = insightParts.length > 0 ? insightParts.join(' ')
    : (totals.expense === 0 && monthExpenseTx.length === 0 && priorMonths.length === 0 ? t.insightNoData : null);

  // Compact expense+income calendar for the selected month
  const [selectedDate, setSelectedDate] = useState(null);
  const [markingId, setMarkingId] = useState(null);
  const [markErrorId, setMarkErrorId] = useState(null);
  const [paidFlashItems, setPaidFlashItems] = useState([]);
  async function handleMarkPaid(r) {
    ...
    try {
      ...
      // Show a brief "paid" state (struck-through, muted) on the row before it disappears from
      // Upcoming, instead of the row vanishing (or seeming to do nothing) with no feedback.
      setPaidFlashItems(prev => [...prev, r]);
      setTimeout(() => setPaidFlashItems(prev => prev.filter(p => p.id !== r.id)), 900);
    } catch (err) {
  ...
  const selectedDayExpense = selectedDayTx.filter(tx => tx.type === 'expense').reduce((s, tx) => s + Number(tx.amount), 0);
        ...
        <>
          ...
          <div className="fc-period-grid">
            ...
            <div className="fc-period-card">
              ...
              <div className="fc-period-amount">{fmtIDR(totals.expense)}</div>
              <div className="fc-period-count">{t.txCount.replace('{n}', monthExpenseTx.length)}</div>
            </div>
          </div>
          ...
          <div className="fc-section">
            ...
            {upcoming.length === 0 && paidFlashItems.length === 0 ? (
              <div className="fc-section-empty">{t.noUpcoming}</div>
            ) : (
              <>
                <div className="fc-upcoming-list">
                  {paidFlashItems.map(r => (
                    <div key={`paid-${r.id}`} className="fc-upcoming-item fc-upcoming-item-paid">
                      <div className="fc-upcoming-main">
                        <div className="fc-upcoming-label">{r.source === 'debt' ? r.debtName : (t.categories[r.category] || r.category)}</div>
                      </div>
                      <div className="fc-upcoming-paid-badge"><CheckCircle2 size={14} /> {t.markAsPaid}</div>
                    </div>
                  ))}
                  {upcoming.filter(r => !paidFlashItems.some(p => p.id === r.id)).slice(0, 4).map(r => {
