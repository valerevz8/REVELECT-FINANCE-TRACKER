...
const T = {
  id: {
    ...
    namePromptTitle: 'Nama kamu siapa?', namePromptPlaceholder: 'Contoh: Vaza', namePromptSave: 'Simpan', namePromptSkip: 'Lewati untuk sekarang',
  ...
  en: {
    ...
    namePromptTitle: "What's your name?", namePromptPlaceholder: 'e.g: Vaza', namePromptSave: 'Save', namePromptSkip: 'Skip for now',
...
function FinanceApp({ session, theme, onThemeChange, subInfo }) {
  ...
  const [namePromptSkipped, setNamePromptSkipped] = useState(false);
  const [showNamePrompt, setShowNamePrompt] = useState(false);

  useEffect(() => {
    if (!loaded) return;
    if (displayName || namePromptSkipped) { setShowNamePrompt(false); return; }
    setShowNamePrompt(true);
  }, [loaded, displayName, namePromptSkipped]);

  async function saveDisplayName(name) {
    setDisplayName(name);
    setShowNamePrompt(false);
    const { error } = await supabase.from('finance_settings').upsert({ user_id: userId, display_name: name }, { onConflict: 'user_id' });
    if (error) console.error('saveDisplayName failed:', error);
  }
  async function skipNamePrompt() {
    setNamePromptSkipped(true);
    setShowNamePrompt(false);
    const { error } = await supabase.from('finance_settings').upsert({ user_id: userId, name_prompt_skipped: true }, { onConflict: 'user_id' });
    if (error) console.error('skipNamePrompt failed:', error);
  }
  ...
  useEffect(() => {
    (async () => {
      ...
      setNamePromptSkipped(!!settingsRes.data?.name_prompt_skipped);
  ...
  return (
    <div className="fc-app" data-theme={theme}>
      ...
      {showNamePrompt && <NamePromptModal t={t} onSave={saveDisplayName} onSkip={skipNamePrompt} />}
...
function NamePromptModal({ t, onSave, onSkip }) {
  ...
  return (
    <div className="fc-modal-backdrop">
      <form className="fc-modal" onClick={(e) => e.stopPropagation()} onSubmit={handleSubmit} style={{ maxWidth: 320 }}>
        <div className="fc-modal-header"><span>{t.namePromptTitle}</span></div>
        <label className="fc-field">
          <input type="text" value={name} onChange={(e) => setName(e.target.value)} placeholder={t.namePromptPlaceholder} autoFocus />
        </label>
        <div className="fc-modal-actions">
          <button type="button" className="fc-btn-ghost" onClick={onSkip}>{t.namePromptSkip}</button>
          <button type="submit" className="fc-btn-primary">{t.namePromptSave}</button>
        </div>
      </form>
    </div>
  );
