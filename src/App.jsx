...
const T = {
  id: {
    ...
    splitBill: 'Split Bill (AI)', splitBillDesc: 'Upload foto bon/struk, terus kasih tau AI item mana yang punya kamu. Sisanya (pajak/service) dihitung proporsional otomatis.',
    myItemsLabel: 'Item kamu (bebas nulisnya)', myItemsPlaceholder: 'Contoh: nasi goreng sama es teh doang',
  ...
  en: {
    ...
    splitBill: 'Split Bill (AI)', splitBillDesc: 'Upload a photo of the receipt, then tell the AI which item(s) are yours. Tax/service charge is calculated proportionally automatically.',
    myItemsLabel: 'Your item(s) (write freely)', myItemsPlaceholder: 'e.g: just the fried rice and iced tea',
...
function FinanceApp({ session, theme, onThemeChange, subInfo }) {
  ...
  return (
    <div className="fc-app" data-theme={theme}>
      ...
      {showSplitBillModal && (
        <SplitBillModal t={t} lang={lang} onClose={() => setShowSplitBillModal(false)}
      ...
      <div className="fc-fab-wrap">
        {showFabMenu && (
          <div className="fc-fab-menu">
            <button className="fc-fab-mini" onClick={() => { setShowFabMenu(false); setShowSplitBillModal(true); }}>
              <Sparkles size={15} /> <span>{t.splitBill}</span>
            </button>
            <button className="fc-fab-mini" onClick={() => { setShowFabMenu(false); setShowQuickAddModal(true); }}>
              <Sparkles size={15} /> <span>{t.quickAdd}</span>
            </button>
            <button className="fc-fab-mini fc-fab-mini-primary" onClick={() => { setShowFabMenu(false); setEditingTx(null); setShowTxModal(true); }}>
              <Plus size={15} /> <span>{t.addTransaction}</span>
            </button>
          </div>
        )}
        <button className="fc-fab-main" onClick={() => setShowFabMenu(s => !s)} aria-label={t.addTransaction}>
...
function Dashboard({ t, totals, health, totalDebtRemaining, lang, budgets, onEditBudget, investPlan, onEditInvestPlan, accounts, onAddAccount, onEditAccount, onDeleteAccount }) {
...
function SplitBillModal({ t, lang, onClose, onParsed }) {
  ...
  function handleConfirm() {
    ...
    onParsed({
      ...
      date: todayISO(),
      note: result.note || (Array.isArray(result.items) ? result.items.join(', ') : ''),
      debtId: null,
    });
  }
...
.fc-fab-main { width: 58px; height: 58px; border-radius: 50%; background: var(--fc-gold, #c9a977); color: #14100c; border: none; display: flex; align-items: center; justify-content: center; box-shadow: 0 6px 18px rgba(0,0,0,0.45); cursor: pointer; flex-shrink: 0; }
.fc-fab-menu { display: flex; flex-direction: column; align-items: flex-end; gap: 10px; }
.fc-fab-mini { display: flex; align-items: center; gap: 8px; background: var(--fc-surface); border: 1px solid var(--fc-border); color: var(--fc-text); padding: 11px 18px; border-radius: 24px; box-shadow: 0 3px 12px rgba(0,0,0,0.35); font-size: 13px; font-weight: 600; white-space: nowrap; cursor: pointer; }
...
.fc-split-item-row { font-size: 13.5px; color: var(--fc-text); }
