...
function FinanceApp({ session, theme, onThemeChange, subInfo }) {
  ...
  useEffect(() => {
    if (!loaded) return;
    if (displayName) { setShowNamePrompt(false); return; }
    try {
      const skipped = localStorage.getItem(`fc-name-skipped-${userId}`);
      if (!skipped) setShowNamePrompt(true);
    } catch (e) { /* ignore */ }
  }, [loaded, displayName, userId]);
  ...
  function skipNamePrompt() {
    setShowNamePrompt(false);
    try { localStorage.setItem(`fc-name-skipped-${userId}`, '1'); } catch (e) { /* ignore */ }
  }
