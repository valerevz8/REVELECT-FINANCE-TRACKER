...
import { Plus, Trash2, TrendingUp, TrendingDown, Wallet, CreditCard, PiggyBank, PieChart as PieIcon, Table as TableIcon, Globe, X, ChevronLeft, ChevronRight, ClipboardList, LayoutDashboard, AlertCircle, CheckCircle2, LogOut } from 'lucide-react';
...
export default function App() {
  ...
  if (session === undefined) {
    return <div style={{ background: '#0d0b09', color: '#c9a977', height: '100vh', display: 'flex', alignItems: 'center', justifyContent: 'center', fontFamily: 'Calibri, sans-serif' }}>...</div>;
  }
  if (!session) {
  ...
  return (
    <div className="fc-authwrap">
      ...
      <form className="fc-modal" style={{ maxWidth: 340 }} onSubmit={handleSubmit}>
        ...
        <label className="fc-field"><span>{t.password}</span><input type="password" required minLength={6} value={password} onChange={(e) => setPassword(e.target.value)} /></label>
...
function FinanceApp({ session }) {
  ...
  if (!loaded) {
    return (
      <div style={{ background: '#0d0b09', color: '#c9a977', height: '100vh', display: 'flex', alignItems: 'center', justifyContent: 'center', fontFamily: 'Calibri, sans-serif' }}>
        {t.loadingApp}
      </div>
    );
  }
  ...
  return (
    <div className="fc-app">
      <style>{CSS}</style>
      <Header t={t} lang={lang} setLang={changeLang} onLogout={() => supabase.auth.signOut()} />
...
--------------------------------------------------------- */
function Header({ t, lang, setLang, onLogout }) {
  return (
      ...
      <div className="fc-gauge-wrap">
        <svg width="160" height="160" viewBox="0 0 160 160">
          <g transform="rotate(135 80 80)">
            <circle cx="80" cy="80" r={r} fill="none" stroke="#2a231c" strokeWidth="10" strokeDasharray={`${totalArc} ${circumference}`} strokeLinecap="round" />
            <circle cx="80" cy="80" r={r} fill="none" stroke={color} strokeWidth="10" strokeDasharray={`${filled} ${circumference}`} strokeLinecap="round" />
          </g>
          <text x="80" y="76" textAnchor="middle" fontSize="30" fontFamily="Calibri" fill="#f0ece4" fontWeight="600">{score}</text>
          <text x="80" y="98" textAnchor="middle" fontSize="10" fontFamily="Calibri" fill="#9b9186">/ 100</text>
        </svg>
      </div>
            ...
            <ResponsiveContainer width="100%" height={240}>
              <PieChart>
                <Pie data={pieData} dataKey="value" nameKey="name" cx="50%" cy="50%" innerRadius={55} outerRadius={90} paddingAngle={2}>
                  {pieData.map((entry) => <Cell key={entry.key} fill={CAT_COLORS[entry.key] || '#786b5a'} stroke="#0d0b09" strokeWidth={1} />)}
                </Pie>
                <Tooltip formatter={(v) => fmtIDR(v)} contentStyle={{ background: '#17130f', border: '1px solid #2a231c', borderRadius: 8, color: '#f0ece4' }} />
              </PieChart>
            </ResponsiveContainer>
            ...
            <tbody>
              {sorted.map(tx => (
                <tr key={tx.id}>
                  ...
                  <td style={{ textAlign: 'right', color: tx.type === 'income' ? '#7c9a72' : '#f0ece4' }}>{tx.type === 'income' ? '+' : '-'}{fmtIDR(tx.amount)}</td>
          ...
          <ResponsiveContainer width="100%" height={260}>
            <PieChart>
              <Pie data={expenseData} dataKey="value" nameKey="name" cx="50%" cy="50%" outerRadius={95} label={({ name, percent }) => `${name} ${(percent * 100).toFixed(0)}%`} labelLine={false}>
                {expenseData.map(entry => <Cell key={entry.key} fill={CAT_COLORS[entry.key] || '#786b5a'} stroke="#0d0b09" strokeWidth={1} />)}
              </Pie>
              <Tooltip formatter={(v) => fmtIDR(v)} contentStyle={{ background: '#17130f', border: '1px solid #2a231c', borderRadius: 8, color: '#f0ece4' }} />
            </PieChart>
          </ResponsiveContainer>
        ...
        {barData.length === 0 ? <div className="fc-empty">{t.noData}</div> : (
          <ResponsiveContainer width="100%" height={260}>
            <BarChart data={barData} layout="vertical" margin={{ left: 10, right: 20 }}>
              <CartesianGrid strokeDasharray="3 3" stroke="#2a231c" horizontal={false} />
              <XAxis type="number" tick={{ fill: '#9b9186', fontSize: 11 }} tickFormatter={(v) => fmtIDR(v)} />
              <YAxis type="category" dataKey="name" tick={{ fill: '#e6e0d5', fontSize: 12 }} width={90} />
              <Tooltip formatter={(v) => fmtIDR(v)} contentStyle={{ background: '#17130f', border: '1px solid #2a231c', borderRadius: 8, color: '#f0ece4' }} />
...
.fc-app { background: #0d0b09; color: #f0ece4; font-family: 'Calibri','Segoe UI',sans-serif; min-height: 100vh; }
.fc-authwrap { background: #0d0b09; min-height: 100vh; display:flex; align-items:center; justify-content:center; padding: 16px; font-family: 'Calibri','Segoe UI',sans-serif; }
.fc-header { display:flex; align-items:center; justify-content:space-between; padding: 20px 22px 16px; border-bottom: 1px solid #2a231c; }
.fc-brand { display:flex; align-items:center; gap: 12px; }
.fc-logo { height: 28px; width: auto; display:block; }
.fc-title { font-size: 22px; letter-spacing: 2px; color: #e0c396; text-transform: uppercase; line-height:1.1; }
.fc-lang-toggle { display:flex; align-items:center; gap:6px; background: #17130f; border: 1px solid #2a231c; color: #9b9186; padding: 7px 12px; border-radius: 20px; font-size: 12px; cursor:pointer; letter-spacing:0.5px; }
.fc-lang-active { color: #c9a977; font-weight: 700; }
.fc-lang-sep { opacity: 0.4; }
.fc-tabs { display:flex; gap: 4px; padding: 10px 22px 0; overflow-x:auto; border-bottom: 1px solid #2a231c; }
.fc-tab { display:flex; align-items:center; gap:6px; background:none; border:none; color:#9b9186; padding: 9px 14px; font-size: 13px; cursor:pointer; border-bottom: 2px solid transparent; white-space:nowrap; }
...
.fc-monthnav button { background: #17130f; border:1px solid #2a231c; color:#c9a977; border-radius:6px; padding:5px 8px; cursor:pointer; display:flex; }
.fc-monthlabel { font-size: 17px; letter-spacing: 1px; color: #f0ece4; min-width: 160px; text-align:center; }
.fc-body { padding: 4px 22px 20px; max-width: 1000px; margin: 0 auto; }
.fc-footer { text-align:center; font-size: 10.5px; color: #9b9186; padding: 10px 0 18px; letter-spacing:0.4px; }
.fc-card { background: #17130f; border: 1px solid #2a231c; border-radius: 12px; padding: 18px; margin-bottom: 16px; }
.fc-card-header { display:flex; align-items:center; justify-content:space-between; margin-bottom: 14px; flex-wrap:wrap; gap:10px; }
.fc-card-title { font-size: 17px; letter-spacing: 0.5px; color: #f0ece4; }
.fc-card-subtitle { font-size: 12px; text-transform:uppercase; letter-spacing:1px; color:#9b9186; margin: 12px 0 8px; }
.fc-empty { color: #9b9186; font-size: 13px; padding: 24px 0; text-align:center; }
...
.fc-gauge-title { font-size: 11px; color: #9b9186; margin-top:6px; text-transform:uppercase; letter-spacing:0.6px; }
.fc-gauge-explain { font-size: 10.5px; color: #9b9186; margin-top:8px; line-height:1.4; }
.fc-gauge-empty { color: #9b9186; font-size:13px; }
...
.fc-stat-label { font-size: 10.5px; color: #9b9186; text-transform:uppercase; letter-spacing:0.5px; }
.fc-stat-value { font-family: ui-monospace, monospace; font-size: 15px; color: #f0ece4; margin-top:2px; font-weight:600; }
...
.fc-legend-name { flex:1; color: #f0ece4; }
.fc-legend-value { font-family: ui-monospace, monospace; color: #9b9186; }
...
.fc-btn-ghost { background: none; border: 1px solid #2a231c; color: #9b9186; padding: 7px 12px; border-radius: 8px; font-size: 12.5px; cursor:pointer; }
.fc-icon-btn { background:none; border:none; color: #9b9186; cursor:pointer; padding: 4px; display:flex; }
.fc-icon-btn:hover { color: #b0584f; }
.fc-table-wrap { overflow-x:auto; }
.fc-table { width:100%; border-collapse: collapse; font-size: 13px; }
.fc-table th { text-align:left; color: #9b9186; font-weight:500; font-size:11px; text-transform:uppercase; letter-spacing:0.5px; padding: 8px 10px; border-bottom: 1px solid #2a231c; }
.fc-table td { padding: 10px; border-bottom: 1px solid #2a231c; color: #f0ece4; }
.fc-note { color: #9b9186; max-width: 160px; overflow:hidden; text-overflow:ellipsis; white-space:nowrap; }
...
.fc-debt-item { border: 1px solid #2a231c; border-radius: 10px; padding: 14px; }
...
.fc-debt-bar-wrap { height: 6px; background: #1f1a15; border-radius: 4px; overflow:hidden; margin-bottom: 8px; }
.fc-debt-bar { height:100%; background: linear-gradient(90deg, #c9a977, #4f8a8b); }
.fc-debt-meta { display:flex; justify-content:space-between; font-size: 12px; color: #9b9186; flex-wrap:wrap; gap:6px; }
...
.fc-delta { display:flex; align-items:center; gap:5px; font-size: 13px; padding: 6px 10px; border-radius: 8px; background: #1f1a15; }
...
.fc-metricbar-label { display:flex; justify-content:space-between; font-size: 12px; color: #9b9186; margin-bottom:4px; }
.fc-metricbar-track { height: 6px; background: #1f1a15; border-radius:4px; overflow:hidden; }
.fc-metricbar-fill { height:100%; }
.fc-tips { display:flex; flex-direction:column; gap: 8px; }
.fc-tip-item { display:flex; gap:8px; align-items:flex-start; font-size: 13px; line-height:1.5; color: #f0ece4; background: #1f1a15; padding: 10px 12px; border-radius: 8px; }
...
.fc-modal { background: #17130f; border: 1px solid #2a231c; border-radius: 14px; padding: 20px; width: 100%; max-width: 380px; display:flex; flex-direction:column; gap: 12px; max-height: 90vh; overflow-y:auto; }
.fc-modal-header { display:flex; align-items:center; justify-content:space-between; font-size: 18px; color: #e0c396; letter-spacing:0.5px; }
.fc-type-toggle { display:grid; grid-template-columns: 1fr 1fr; gap: 6px; }
.fc-type-btn { background: #1f1a15; border: 1px solid #2a231c; color: #9b9186; padding: 8px 6px; border-radius: 8px; font-size: 12px; cursor:pointer; }
.fc-type-btn-active { border-color: #c9a977; color: #c9a977; background: rgba(201,169,119,0.08); }
.fc-field { display:flex; flex-direction:column; gap: 5px; font-size: 12px; color: #9b9186; }
.fc-field input, .fc-field select { background: #1f1a15; border: 1px solid #2a231c; color: #f0ece4; padding: 9px 10px; border-radius: 8px; font-size: 14px; font-family: 'Calibri','Segoe UI',sans-serif; }
.fc-field input:focus, .fc-field select:focus { outline: 1.5px solid #c9a977; }
.fc-field-note { font-size: 12px; color: #9b9186; }
